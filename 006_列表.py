...
'''
1. 列表是啥？
    (1) 列表是一个有序集合，由一系列按照特定顺序排列的元素组成
    (2) 列表的元素可以是数字、字符、字符串等等
    (3) 用"[]"来表示列表，用","来分隔列表元素
    (4) 同一个列表，元素可以为不同类型

2. 如何访问列表元素？
    (1) 类似于访问数组元素，可以使用 “列表名+索引” 的方式访问列表元素。
    (2) 索引从0开始，python中的索引可以为负数。例如-1表示列表的最后一个元素，-2表示列表的倒数第2个元素
    (3) 注意，索引不能超过列表长度
...
4. 如何在列表中新增元素？
    (1) append()，在列表末尾追加元素
    (2) insert()，在列表的指定位置前插入一个元素。索引值可以大于等于列表长度

5. 如何删除列表元素？
    (1) del语句，删除列表指定位置的元素
    (2) pop()，删除列表指定位置的元素。如果不指定位置，默认为列表末尾的元素。pop删除的元素，可以用一个变量来保存
    (3) remove()，根据元素的值来删除元素。如果要删除的元素有多个，只会删除第一个；如果需要删除多个，需要多次删除

6. 列表和数组、链表的异同点？
    (1) 列表和数组：可以通过索引来访问指定位置的元素；但列表的索引可以为负数，数组的索引必须非负
    (2) 列表和链表：可以动态的增加和删除
'''
